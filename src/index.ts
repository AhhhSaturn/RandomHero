...
commandMap.set("random", (interaction) => {
	...
	const isStadium = interaction.options.getBoolean("stadium");

	const embeds: EmbedBuilder[] = [];
	const files: AttachmentBuilder[] = [];

	const mainHero = getRandomHero(role, !!isStadium);
	embeds.push(mainHero?.embed);
	files.push(mainHero?.file);

	if (interaction.options.getBoolean("backup")) {
		const backupHero = getRandomHero(mainHero.role, !!isStadium);
